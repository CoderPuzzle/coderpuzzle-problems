...
impl Solution {
    pub fn product_of_rest(nums: Vec<i32>) -> Vec<CoderPuzzleBig> {
        ...
        let mut pre = vec![CoderPuzzleBig::one(); n + 1];
        for i in 0..n {
            pre[i + 1] = pre[i].mul_small(nums[i]);
        }
        // suf[i] = product of everything from index i onward.
        let mut suf = vec![CoderPuzzleBig::one(); n + 1];
        for i in (0..n).rev() {
            suf[i] = suf[i + 1].mul_small(nums[i]);
        }
        // pre[i] x suf[i+1] spans everything except nums[i] itself; a lone
        // zero zeroes every cell but its own, automatically.
        (0..n).map(|i| pre[i].mul_big(&suf[i + 1])).collect()
    }
}
