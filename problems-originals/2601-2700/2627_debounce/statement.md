...
timer whose deadline has been reached before each call proceeds (earlier
deadlines first, exactly as Node drains its timer queue), then runs every
surviving timer to completion. Each actual execution of your wrapped fn
records one output row `{"t": <virtual execution time>, "inputs": [...]}`
in execution order; that recorded transcript is the judged answer shown as
`Output` below. Never bypass the timers (no setImmediate or synchronous
calls) — only executions scheduled through setTimeout/clearTimeout count.
