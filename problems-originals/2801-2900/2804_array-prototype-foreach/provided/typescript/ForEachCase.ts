...
//   ForEachCase exposes the problem's one judged invocation: .arr is the
...
class ForEachCase {
    ...
    constructor(values: any[], _queryBudget?: unknown) {
        ...
        // byte-exact.
        const bound = (source as string).replace(
            /^(\s*)(async\s+)?\(([^()]*)\)\s*=>\s*([\s\S]+)$/,
            (_all: string, pad: string, asyncWord: string | undefined, params: string, body: string) =>
                `${pad}${asyncWord || ""}function (${params}) ` +
                (body.trimStart().startsWith("{") ? body.trim() : `{ return ${body.trim()}; }`),
        );
        this.fn = new Function("return (" + bound + ");")() as unknown as (...args: any[]) => any;
    }
